...
def encrypt(ptext):
    ...
    print("After Pre-round Transformation: ", conv_to_hexa(state))

    print("Round key K0: ", conv_to_hexa((w[0] << 8) + w[1]))

    ## -----------Round1 Start----------
    state = sub4NibList(sBox, state)                                        # Substitute nibbles
    print("After Round 1 Substitute nibbles: ", conv_to_hexa(state))
    state = shiftRow(state)                                                 # Shift rows
    print("After Round 1 Shift rows: ", conv_to_hexa(state))
    state = mixCol(state)                                                   # Mix columns
    print("After Round 1 Mix columns: ", conv_to_hexa(state))
    state = addKey(intToVec((w[2] << 8) + w[3]), state)                     # Add Round 1 Key
    print("After Round 1 Add round key: ", conv_to_hexa(state))
    ## -----------Round1 End----------

    print("Round key K1: ", conv_to_hexa((w[2] << 8) + w[3]))

    ## ------------Round2 Start---------
    state = sub4NibList(sBox, state)                                        # Substitute nibbles
    print("After Round 2 Substitute nibbles: ", conv_to_hexa(state))
    state = shiftRow(state)                                                 # Shift rows
    print("After Round 2 Shift rows: ", conv_to_hexa(state))
    state = vecToInt(addKey(intToVec((w[4] << 8) + w[5]), state))           # Add Round 2 Key
    print("After Round 2 Add round key: ", conv_to_hexa(state))
    ## ----------Round2 End---------

    print("Round Key K2: ", conv_to_hexa((w[4] << 8) + w[5]))
    print("Cipher text: ", conv_to_hexa(state))
    print()
...
def decrypt(ctext):
    ...
    print("After Round 1 InvShift rows:: ", conv_to_hexa(state))
    state = sub4NibList(sBoxI, state)                                       # Inverse Nibble Sub
    print("After Round 1 InvSubstitute nibbles:: ", conv_to_hexa(state))
    state = addKey(intToVec((w[2] << 8) + w[3]), state)                     # Add Round 1 Key
    print("After Round 1 InvAdd round key:: ", conv_to_hexa(state))

    print("Round key K1:: ", conv_to_hexa((w[2] << 8) + w[3]))
    state = iMixCol(state)                                                  # Inverse Mix Columns
    print("After Round 1 InvMix columns:: ", conv_to_hexa(state))
    ## ------------Round1 Inverse End-----------------

    ## ------------Round2 Inverse Start---------------
    state = shiftRow(state)                                                 # Inverse Shift Row
    print("After Round 2 InvShift rows:: ", conv_to_hexa(state))                          
    state = sub4NibList(sBoxI, state)                                       # Inverse Nibble Sub
    print("After Round 2 InvSubstitute nibbles:: ", conv_to_hexa(state))
    state =  vecToInt(addKey(intToVec((w[0] << 8) + w[1]), state))          # Add Round 0 Key       
    print("After Round 2 Add round key:: ", conv_to_hexa(state))
    ## ------------Round2 Inverse End-----------------

    print("Round Key K0:: ", conv_to_hexa((w[0] << 8) + w[1]))
    print("Plaintext:: ", conv_to_hexa(state))
    print()
